...
class CategoryAwareAIPredictor:
    ...
    def _extract_time_series(self, api_id, hours=48):
        ...
        for log in logs:
            ...
            expected_latency = category_config["expected_latency"]
            normalized_latency = log.get("total_latency_ms", 0) / expected_latency
            
            features = [
                1.0 if log.get("is_up", True) else 0.0,
                min(normalized_latency, 10.0),  # Cap at 10x expected
                log.get("dns_latency_ms", 0) / 100.0,
                log.get("tcp_latency_ms", 0) / 100.0,
                log.get("tls_latency_ms", 0) / 100.0,
                log.get("server_processing_latency_ms", 0) / expected_latency,
                log.get("content_download_latency_ms", 0) / expected_latency,
                log.get("status_code", 200) / 500.0,
    ...
    def predict_failure(self, api_id, hours_ahead=1):
        ...
        try:
            ...
            if self.use_ml:
                ...
                category_config = API_CATEGORIES.get(category, API_CATEGORIES["REST API"])
                adjusted_confidence = prediction * (1.0 / category_config["failure_threshold"])
    ...
    def _statistical_prediction(self, api_id, category):
        ...
        expected_failure_rate = category_config["failure_threshold"]
